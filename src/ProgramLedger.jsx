...
function LineageManager({ raw, legacyKeys, setLegacyKeys, lineage, setLineage, onClose }) {
  ...
  function addRetire() {
    if (!retireSel) return;
    setLegacyKeys(prev => new Set(prev).add(retireSel));
    setRetireSel("");
  }
  function removeRetire(key) {
    setLegacyKeys(prev => { const n = new Set(prev); n.delete(key); return n; });
  }
  function addLink() {
    if (!fromSel || !toSel || fromSel === toSel) return;
    setLineage(prev => [...prev, { fromKey: fromSel, toKey: toSel, note, fromLabel: rowLabel(options.find(o => rowKey(o)===fromSel)), toLabel: rowLabel(options.find(o => rowKey(o)===toSel)) }]);
    setFromSel(""); setToSel(""); setNote("");
  }
  function removeLink(idx) {
    setLineage(prev => prev.filter((_,i) => i !== idx));
  }
        ...
        <div className="p-6 space-y-8">
          ...
          <section className="border-t pt-6" style={{ borderColor: '#DDD7C8' }}>
            ...
            {lineage.length === 0 ? (
              ...
              <div className="flex flex-col gap-1.5">
                {lineage.map((l,i) => (
                  <div key={i} className="flex items-center justify-between text-xs font-sans px-3 py-2 rounded" style={{ background: '#F1EEE3', border: '1px solid #C9C4B4' }}>
                    <span style={{ color: '#33312D' }}>{l.fromLabel} &rarr; {l.toLabel}{l.note ? ` (${l.note})` : ''}</span>
                    <button onClick={() => removeLink(i)} className="font-mono" style={{ color: '#B8462F' }}>remove</button>
                  </div>
                ))}
              </div>
            )}
