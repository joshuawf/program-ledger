...
function applyLineageToRawRows(enrollmentRows, graduationRows, lineage) {
  if (lineage.length === 0) return { enrollmentRows, graduationRows, predecessorMap: new Map() };
              ...
              <>
                ...
                {!row.bothSidesFlagged && (
                  <div className="text-xs font-sans p-2 rounded" style={{ background: '#F1EEE3', color: '#6B6455', border: '1px solid #C9C4B4' }}>
                    Only flagged on {row.hasEnrFlag ? 'the enrollment' : 'the graduation'} side — the ledger only marks a program as "flagged" (the stamp badge) when it's low on <em>both</em> enrollment and graduation. This one's worth knowing about, but doesn't count toward the flagged total.
                  </div>
                )}
              </>
            )}
...
function UploadPanel({ onClose, onImported }) {
  ...
  async function confirmImport() {
    ...
    setSaving(true);
    setError(null);
    const table = fileType === 'enrollment' ? 'enrollment' : 'graduation';
    const rows = parsed.map(r => fileType === 'enrollment'
      ? { location: String(r.location), program: String(r.program), degree: String(r.degree), period: String(r.period), period_sort: Number(r.period_sort), count: Number(r.count) }
      : { location: String(r.location), program: String(r.program), degree: String(r.degree), year: Number(r.year), count: Number(r.count) }
    );
  ...
  const totalNew = parsed ? parsed.reduce((s, r) => s + (Number(r.count) || 0), 0) : 0;
      ...
      <div className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-lg shadow-2xl" style={{ background: '#EEF1EC' }} onClick={e => e.stopPropagation()}>
        <div className="p-6 flex items-start justify-between" style={{ background: '#1B2A41' }}>
          <div>
            ...
            <h2 className="font-serif text-xl" style={{ color: '#F1F0EA' }}>Upload enrollment or graduation counts</h2>
          </div>
          <button onClick={onClose} className="text-xs font-sans" style={{ color: '#A9B7C8' }}>&times; Close</button>
        </div>
...
export default function ProgramLedger() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      const firstErr = enrRes.error || gradRes.error || retRes.error || linkRes.error || foundRes.error;
  ...
  const { enrollmentRows, graduationRows, predecessorMap } = useMemo(
    () => applyLineageToRawRows(rawEnrollment, rawGraduation, lineage),
    [rawEnrollment, rawGraduation, lineage]
  );

  const allDerived = useMemo(() => {
    const derived = computeDerivedRows({ enrollmentRows, graduationRows, foundingOverrides });
    ...
    const overallDerived = computeDerivedRows({ enrollmentRows: overallEnr, graduationRows: overallGrad, foundingOverrides })
      .map(r => ({ ...r, predecessors: [] }));

    return [...withPredecessors, ...overallDerived];
  }, [enrollmentRows, graduationRows, foundingOverrides, predecessorMap]);
  ...
  const stats = useMemo(() => {
    ...
    const enrOnly = flagged.filter(r => r.hasEnrFlag && !r.hasGradFlag).length;
    const gradOnly = flagged.filter(r => r.hasGradFlag && !r.hasEnrFlag).length;
    return { total, flaggedCount, enrOnly, gradOnly };
  }, [flagged]);
    ...
    <div className="flex min-h-screen w-full font-sans" style={{ background: '#EEF1EC' }}>
      ...
      <aside className="w-72 shrink-0 p-6 flex flex-col gap-7" style={{ background: '#1B2A41' }}>
        ...
        <div className="mt-auto pt-5 border-t text-[11px] font-sans leading-relaxed" style={{ borderColor: '#33425C', color: '#7A8699' }}>
          ...
          A program earns a flag reason when its 5-yr average enrollment or graduate count is &le;10, or falls in the bottom 10% of whatever's currently in view. But the flagged stamp only shows up when a program is low on <strong>both</strong> enrollment and graduation — being small on just one side isn't enough. The Winter window and graduation window both roll forward automatically as new data is added.
          <br /><br />
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Flagged (low on both)', value: stats.flaggedCount, color: '#B8462F' },
            { label: 'Enrollment concern only', value: stats.enrOnly, color: '#8A8F86' },
            { label: 'Graduation concern only', value: stats.gradOnly, color: '#8A8F86' },
          ].map((s, i) => (
